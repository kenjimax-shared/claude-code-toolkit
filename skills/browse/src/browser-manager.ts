...
export class BrowserManager {
  ...
  private _onCrash: (() => void) | null = null;

  // ─── Ref Map (snapshot → @e1, @e2, ...) ────────────────────
  private refMap: Map<string, Locator> = new Map();

  /** Register a callback to invoke on Chromium crash (for graceful shutdown) */
  onCrash(fn: () => void) {
    this._onCrash = fn;
  }
  ...
  async launch() {
    ...
    // Chromium crash → trigger graceful shutdown via callback
