...
// ─── Orphan Reaper ──────────────────────────────────────────────
// Kill stale bun server processes from prior sessions that have no live Chromium child
async function reapOrphanServers() {
  try {
    const { execSync } = await import('child_process');
    // Find all bun processes running server.ts (excluding ourselves)
    const myPid = process.pid;
    const result = execSync(
      `ps -eo pid,ppid,args 2>/dev/null | grep 'bun.*server\\.ts' | grep -v grep || true`,
      { encoding: 'utf-8' }
    ).trim();
    if (!result) return;
    ...
    for (const line of result.split('\n')) {
      ...
      // Check if this process has a live Chromium child
      const children = execSync(
        `ps --ppid ${pid} -o args= 2>/dev/null || true`,
        { encoding: 'utf-8' }
      ).trim();
      const hasChromium = children.split('\n').some(c => c.includes('chromium') || c.includes('chrome'));

      if (!hasChromium) {
        console.log(`[browse] Reaping orphan server PID ${pid} (no Chromium child)`);
        try { process.kill(pid, 'SIGTERM'); } catch {}
      }
    }
  } catch (err: any) {
    // Non-fatal: reaper is best-effort
    console.log(`[browse] Orphan reaper: ${err.message}`);
  }
}

// ─── Safety Net Handlers ────────────────────────────────────────
process.on('uncaughtException', (err) => {
  console.error(`[browse] Uncaught exception: ${err.message}`);
  try { fs.unlinkSync(STATE_FILE); } catch {}
  process.exit(1);
});
...
async function start() {
  // Clean up orphaned servers from prior sessions before binding port
  ...
  // Launch browser and wire crash handler for graceful shutdown
  await browserManager.launch();
  browserManager.onCrash(() => shutdown());
